...
@dag( #decorator que vai dar funcionalidade a função abaixo
    dag_id='postgres_to_snowflake',
    default_args=default_args,
    description='Load data incrementally from Postgres to Snowflake',
    schedule_interval=timedelta(days=1),
    catchup=False #Falso quer dizer que ele nao vai tentar executar alguma task que não foi executada
)
def postgres_to_snowflake_etl(): #extende a funcionalidade do decorator
    ...
    for table_name in table_names:
        ...
        def get_max_primary_key(table_name: str):
            # as proximas linhas sera uma conexão com o banco de dados do snowflake
            with SnowflakeHook(snowflake_conn_id='snowflake').get_conn() as conn:
                with conn.cursor() as cursor:
                    ...
                    max_id = cursor.fetchone()[0] #retorne a info da coluna 1[0] para eu manipulá-la
        ...
        def load_incremental_data(table_name: str, max_id: int):
            with PostgresHook(postgres_conn_id='postgres').get_conn() as pg_conn:
                with pg_conn.cursor() as pg_cursor:
                    ...
                    #O information_schema é uma tabela de um db que contem info
                    pg_cursor.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = '{table_name}'")
                    columns = [row[0] for row in pg_cursor.fetchall()] #pegando apenas o nome de cada coluna
                    columns_list_str = ', '.join(columns) #convertendo as colunas para ser usado em modo consulta
                    placeholders = ', '.join(['%s'] * len(columns)) #sera usado para acrescentar valores
                    
                    pg_cursor.execute(f"SELECT {columns_list_str} FROM {table_name} WHERE {primary_key} > {max_id}")
                    rows = pg_cursor.fetchall() # a partir da consulta anterior, pegue todos os registros
